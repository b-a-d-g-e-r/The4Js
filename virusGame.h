	...
	void playVirus(struct player* currentAccount) {
	    ...
	    currentAccount->attempts[VIRUX_INDEX] = currentAccount->attempts[VIRUX_INDEX] + 1;
	    ...
	    while (alive == 1) {
			...
			if(currentAccount->scores[VIRUX_INDEX] > day)
			{
				currentAccount->scores[VIRUX_INDEX] = day;
			}
			Sleep(3000);
